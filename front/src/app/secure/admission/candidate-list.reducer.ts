...
export function offeredCandidateListReducer(state = initialState, action: Action): CandidateListState {
  switch (action.type) {
    case AdmissionActions.FIND_OFFERED_CANDIDATES_SUCCESS: {
      return action.payload;
    }
    default: {
      return state;
...
export function acceptedCandidateListReducer(state = initialState, action: Action): CandidateListState {
    switch (action.type) {
      case AdmissionActions.FIND_ACCEPTED_CANDIDATES_SUCCESS: {
        return action.payload;
      }
      default: {
        return state;
