...
public class InSubjectCodeDaoImpl extends GenericDaoSupport<Long, InSubjectCode> implements InSubjectCodeDao {
    ...
    public InSubjectCode findByCode(String code) {
        ...
        return (InSubjectCode) query.uniqueResult();
    }

    @Override
    public List<InSubjectCode> find() {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select s from InSubjectCode s where " +
                " s.metadata.state = :state ");
        query.setInteger("state", InMetaState.ACTIVE.ordinal());
        query.setCacheable(true);
        return (List<InSubjectCode>) query.list();
    }

    @Override
    public List<InSubjectCode> find(String filter, Integer offset, Integer limit) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select s from InSubjectCode s where " +
    ...
    public Integer count(String filter) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select count(s) from InSubjectCode s where " +
                "(upper(s.code) like upper(:filter) " +
                "or upper(s.description) like upper(:filter)) " +
                "and s.metadata.state = :state ");
        query.setString("filter", WILDCARD + filter + WILDCARD);
    ...
    public boolean isExists(String code) {
        Session session = sessionFactory.getCurrentSession();
        Query query = session.createQuery("select count(*) from InSubjectCode s where " +
                "s.code = :code " +
                "and s.metadata.state = :state ");
        query.setString("code", code);
        query.setInteger("state", InMetaState.ACTIVE.ordinal());
